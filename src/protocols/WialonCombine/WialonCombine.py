...
class WialonCombine:
    ...
    def handle_packet(self):
        ...
        while not self.stop:
            ...
            if packet_header['type']==0:
                ...
                while len(packet)>4:
                    try:
                        ...
                        logger.error(f'[WialonCombine] error parsing {packet} {str(e)}')
                        break
            else:
                logger.error(f'[WialonCombine] Unknown type {packet}')
                continue
            ...
            if self.success:
                ...
                resp = pack(resp)
                logger.debug(f'[WialonCombine] response to tracker:\n{resp}\n')
                self.sock.send(resp)
                if count:
    ...
    def handle_header(self, packet):
        ...
        logger.debug(f'[WialonCombine] header data: {data}')
        return packet, data
    ...
    def send_command(self, command):
        ...
        crc16_pack = struct.unpack(f'{len(packet[:-2])//2}s', binascii.a2b_hex(packet[:-2].encode('ascii')))[0]
        packet = add_ushort(packet, crc16(crc16_pack))
        logger.debug(f'[WialonCombine] командный пакет сформирован:\n{packet}\n')
        packet = pack(packet)
        self.sock.send(packet)
        logger.debug(f'[WialonCombine]  команда {command} отправлена\n')
